    ...
    def input_producer(self, batch_size=10):
        ...
        with tf.variable_scope('input'):
            ...
            self.data_queue = tf.train.slice_input_producer(
                [in_list, gt_list, ed_list], capacity=1000)
            image_in, image_gt, image_ed = read_data()
            batch_in, batch_gt, batch_ed = tf.train.batch(
                [image_in, image_gt, image_ed], batch_size=batch_size, num_threads=16, capacity=1000)
    ...
    def multi_gpu_train(self):
        with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            batch_queue = tf.contrib.slim.prefetch_queue.prefetch_queue(
                [img_batch, img_gt_batch, img_ed_batch], capacity=16)
            tower_grads = []
            # pdb.set_trace()
